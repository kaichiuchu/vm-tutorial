...
void MemoryViewWidget::SetupFromAppSettings() noexcept {
  ...
  const auto debugger_font = app_settings.GetDebuggerFont();
  SetFont(debugger_font);
}

void MemoryViewWidget::UpdateFontMetrics() noexcept {
  const auto font_metrics = fontMetrics();

  data_width_ = font_metrics.horizontalAdvance("FF");
  address_width_ = font_metrics.horizontalAdvance("FFFF");

  char_width_ = font_metrics.horizontalAdvance('0');
  char_height_ = font_metrics.height();

  ascii_start_x_ = ((data_width_ * 2) * bytes_per_line_) + 36;
}

void MemoryViewWidget::DrawAddressDivider(QPainter& painter) noexcept {
  painter.drawLine(address_width_, 0, address_width_, height());
}

void MemoryViewWidget::DrawHeaderDivider(QPainter& painter) noexcept {
  painter.drawLine(0, char_height_, width(), char_height_);
}

void MemoryViewWidget::DrawColumns(QPainter& painter) noexcept {
  auto x = ((data_width_ * 2) + 8);
  auto ascii_x_ = ascii_start_x_;

  for (auto col = 0; col < bytes_per_line_;
       ++col, x += (data_width_ * 2), ascii_x_ += (char_width_ * 2)) {
    if ((col % 2) != 0) {
      painter.fillRect(x, 0, data_width_, height(),
    ...
    painter.drawText(ascii_x_, char_height_, ch);
  }
}
...
void MemoryViewWidget::DrawData(QPainter& painter) noexcept {
  ...
  for (auto row = 0; row <= row_count; ++row, y += char_height_) {
    auto ascii_offset = ascii_start_x_;
    auto x = 0;
