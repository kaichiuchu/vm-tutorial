...
TYPED_TEST(ImplementationTest, Opcode_RET) {
  ...
  //
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  this->impl_.stack_[0] = 0xBEE6;

  // Pretend that the stack pointer is 0.
  this->impl_.stack_pointer_ = 0;
  ...
  // stack.
  ASSERT_EQ(this->impl_.program_counter_, 0xBEE6);

  // Make sure the stack pointer underflowed.
  ASSERT_EQ(this->impl_.stack_pointer_, -1);
}
...
TYPED_TEST(ImplementationTest, Opcode_RET_Detect_StackUnderflow) {
  ...
  ASSERT_EQ(this->impl_.Step(), chip8::StepResult::kStackUnderflow);

  // Make sure that the program counter is trapped at the beginning of the
  // program area.
  ASSERT_EQ(this->impl_.program_counter_,
            chip8::initial_values::kProgramCounter);
}
...
TYPED_TEST(ImplementationTest, Opcode_JP) {
  ...
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers
  InjectInstruction(this->impl_, 0x11, 0x23);

  // Make sure the instruction succeeded.
  ASSERT_EQ(this->impl_.Step(), chip8::StepResult::kSuccess);

  // Make sure the program counter is at 0x0123.
  ASSERT_EQ(this->impl_.program_counter_, 0x0123);
}
...
TYPED_TEST(ImplementationTest, Opcode_CALL) {
  ...
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers
  InjectInstruction(this->impl_, 0x21, 0x23);
  ...
  ASSERT_EQ(this->impl_.stack_pointer_, 0);
  ...
  ASSERT_EQ(this->impl_.stack_[this->impl_.stack_pointer_],
            chip8::initial_values::kProgramCounter + 2);
...
TYPED_TEST(ImplementationTest, Opcode_CALL_Detect_StackOverflow) {
  ...
  //
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  this->impl_.stack_pointer_ = 0xDEFEC8ED;

  // Make sure that the return value of the step is a stack overflow.
  ASSERT_EQ(this->impl_.Step(), chip8::StepResult::kStackOverflow);

  // Make sure the stack pointer wasn't changed.
  ASSERT_EQ(this->impl_.stack_pointer_, 0xDEFEC8ED);
...
TYPED_TEST(ImplementationTest, Opcode_SE_Vx_Imm_BranchTaken) {
  Inject_SE_Vx_Imm(this->impl_, opcode_state::kBranch);
  ...
  ASSERT_EQ(this->impl_.program_counter_,
            chip8::initial_values::kProgramCounter + 4);
}

// NOLINTNEXTLINE(cppcoreguidelines-avoid-non-const-global-variables)
TYPED_TEST(ImplementationTest, Opcode_SE_Vx_Imm_BranchNotTaken) {
  Inject_SE_Vx_Imm(this->impl_, opcode_state::kDoNotBranch);
...
TYPED_TEST(ImplementationTest, Opcode_SNE_Vx_Imm_BranchTaken) {
  Inject_SNE_Vx_Imm(this->impl_, opcode_state::kBranch);
...
TYPED_TEST(ImplementationTest, Opcode_SNE_Vx_Imm_BranchNotTaken) {
  Inject_SNE_Vx_Imm(this->impl_, opcode_state::kDoNotBranch);
...
TYPED_TEST(ImplementationTest, Opcode_SE_Vx_Vy_BranchTaken) {
  Inject_SE_Vx_Vy(this->impl_, opcode_state::kBranch);
...
TYPED_TEST(ImplementationTest, Opcode_SE_Vx_Vy_BranchNotTaken) {
  Inject_SE_Vx_Vy(this->impl_, opcode_state::kDoNotBranch);
...
TYPED_TEST(ImplementationTest, Opcode_LD_Vx_Imm) {
  const auto& V1 = this->impl_.V_[1];

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0x61, 0xFF);
...
TYPED_TEST(ImplementationTest, Opcode_ADD_Vx_Imm) {
  auto& V1 = this->impl_.V_[1];
  V1 = 1;

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0x71, 0xFE);
...
TYPED_TEST(ImplementationTest, Opcode_LD_Vx_Vy) {
  const auto& V0 = this->impl_.V_[0];  // Destination register
  auto& V1 = this->impl_.V_[1];        // Source register

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  V1 = 0xBA;

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0x80, 0x10);
...
TYPED_TEST(ImplementationTest, Opcode_OR_Vx_Vy) {
  const auto [V0, V1] =
      InjectBitInstruction(this->impl_, 0x80, 0x11, 0xB0, 0xB4);
...
TYPED_TEST(ImplementationTest, Opcode_AND_Vx_Vy) {
  const auto [V0, V1] =
      InjectBitInstruction(this->impl_, 0x80, 0x12, 0xB5, 0xB1);
...
TYPED_TEST(ImplementationTest, Opcode_XOR_Vx_Vy) {
  const auto [V0, V1] =
      InjectBitInstruction(this->impl_, 0x80, 0x13, 0xA0, 0xA0);
...
TYPED_TEST(ImplementationTest, Opcode_ADD_Vx_Vy) {
  const auto [V0, VF] = Inject_ADD_Vx_Vy(this->impl_, 0xFE, 0x01);
...
TYPED_TEST(ImplementationTest, Opcode_ADD_Vx_Vy_Detect_Overflow) {
  const auto [V0, VF] = Inject_ADD_Vx_Vy(this->impl_, 0xFE, 0xFA);
...
TYPED_TEST(ImplementationTest, Opcode_SUB_Vx_Vy) {
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  const auto [V0, VF] = Inject_SUB(this->impl_, 0xFF, 0x01);
...
TYPED_TEST(ImplementationTest, Opcode_SUB_Vx_Vy_DetectOverflow) {
  const auto [V0, VF] = Inject_SUB(this->impl_, 0x03, 0x01);
...
TYPED_TEST(ImplementationTest, Opcode_SHR_Vx_LSBClear) {
  const auto [V0, VF] = Inject_SHR(this->impl_, 0xDC);
...
TYPED_TEST(ImplementationTest, Opcode_SHR_Vx_LSBSet) {
  const auto [V0, VF] = Inject_SHR(this->impl_, 0xDD);
...
TYPED_TEST(ImplementationTest, Opcode_SUBN_Vx_Vy_NoBorrow) {
  const auto [V0, VF] = Inject_SUBN(this->impl_, 0x1, 0x03);
...
TYPED_TEST(ImplementationTest, Opcode_SUBN_Vx_Vy_WithBorrow) {
  const auto [V0, VF] = Inject_SUBN(this->impl_, 0x4, 0x3);
...
TYPED_TEST(ImplementationTest, Opcode_SHL_Vx_WithCarry) {
  const auto [V0, VF] = Inject_SHL(this->impl_, 0x90);
...
TYPED_TEST(ImplementationTest, Opcode_SHL_Vx_WithoutCarry) {
  const auto [V0, VF] = Inject_SHL(this->impl_, 0x5A);
...
TYPED_TEST(ImplementationTest, Opcode_SNE_Vx_Vy_BranchTaken) {
  Inject_SNE_Vx_Vy(this->impl_, opcode_state::kBranch);
...
TYPED_TEST(ImplementationTest, Opcode_SNE_Vx_Vy_BranchNotTaken) {
  Inject_SNE_Vx_Vy(this->impl_, opcode_state::kDoNotBranch);
...
TYPED_TEST(ImplementationTest, Opcode_LD_I_Addr) {
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0xA2, 0x45);

  // Make sure the instruction succeded.
  ASSERT_EQ(this->impl_.Step(), chip8::StepResult::kSuccess);

  // I should be 0x245.
  ASSERT_EQ(this->impl_.I_, 0x245);
}

// NOLINTNEXTLINE(cppcoreguidelines-avoid-non-const-global-variables)
TYPED_TEST(ImplementationTest, Opcode_JP_V0_Addr) {
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  this->impl_.V_[0] = 0x20;

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0xB1, 0x23);

  // Make sure the instruction succeded.
  ASSERT_EQ(this->impl_.Step(), chip8::StepResult::kSuccess);

  // Since (0x20 + 0x123) = 0x143, the program counter should be as such.
  ASSERT_EQ(this->impl_.program_counter_, 0x143);
}

// NOLINTNEXTLINE(cppcoreguidelines-avoid-non-const-global-variables)
TYPED_TEST(ImplementationTest, Opcode_RND_Vx_Imm) {
  auto& V0 = this->impl_.V_[0];

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  V0 = 0xF2;

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0xC0, 0x12);
...
TYPED_TEST(ImplementationTest, Opcode_SKP_Vx_BranchNotTaken) {
  Inject_SKP_Vx(this->impl_, opcode_state::kDoNotBranch);
...
TYPED_TEST(ImplementationTest, Opcode_SKNP_Vx_BranchNotTaken) {
  Inject_SKNP_Vx(this->impl_, opcode_state::kDoNotBranch);
...
TYPED_TEST(ImplementationTest, Opcode_LD_Vx_DT) {
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  this->impl_.delay_timer_ = 0xFE;

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0xF0, 0x07);

  // Make sure the instruction succeeded.
  ASSERT_EQ(this->impl_.Step(), chip8::StepResult::kSuccess);

  // Verify that V0 is 0xFE.
  ASSERT_EQ(this->impl_.V_[0], this->impl_.delay_timer_);
}

// NOLINTNEXTLINE(cppcoreguidelines-avoid-non-const-global-variables)
TYPED_TEST(ImplementationTest, Opcode_LD_Vx_K) {
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0xF0, 0x0A);

  // Make sure the instruction succeeded.
  ASSERT_EQ(this->impl_.Step(), chip8::StepResult::kHaltUntilKeyPress);

  // Now press a key to stop waiting for a key press.
  this->impl_.SetKeyState(chip8::Key::k1, chip8::KeyState::kPressed);

  // Make sure that the key "pressed" made it to the V0 register.
  ASSERT_EQ(this->impl_.V_[0], chip8::Key::k1);

  // The virtual machine should no longer be waiting for a key press.
  ASSERT_FALSE(this->impl_.IsHaltedUntilKeyPress());
}

// NOLINTNEXTLINE(cppcoreguidelines-avoid-non-const-global-variables)
TYPED_TEST(ImplementationTest, Opcode_LD_DT_Vx) {
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  this->impl_.V_[0] = 0xDE;

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0xF0, 0x15);

  // Make sure the instruction succeeded.
  ASSERT_EQ(this->impl_.Step(), chip8::StepResult::kSuccess);

  // The delay timer should be 0xDE.
  ASSERT_EQ(this->impl_.delay_timer_, 0xDE);
}
...
TYPED_TEST(ImplementationTest, Opcode_LD_ST_Vx) {
  ...
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0xF0, 0x18);

  // Make sure the instruction succeeded.
  ASSERT_EQ(this->impl_.Step(), chip8::StepResult::kSuccess);

  // The sound timer should be 0xDE.
  ASSERT_EQ(this->impl_.sound_timer_, 0xDE);
}

// NOLINTNEXTLINE(cppcoreguidelines-avoid-non-const-global-variables)
TYPED_TEST(ImplementationTest, Opcode_ADD_I_Vx) {
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  this->impl_.V_[0] = 0x05;

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  this->impl_.I_ = 0xBF;

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0xF0, 0x1E);

  // Make sure the instruction succeeded.
  ASSERT_EQ(this->impl_.Step(), chip8::StepResult::kSuccess);

  // The result should be 0xC4.
  ASSERT_EQ(this->impl_.I_, 0xC4);
}
...
TYPED_TEST(ImplementationTest, Opcode_LD_F_Vx) {
  ...
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0xF0, 0x29);

  // Make sure the instruction succeeded.
  ASSERT_EQ(this->impl_.Step(), chip8::StepResult::kSuccess);

  // I should be 0xF.
  ASSERT_EQ(this->impl_.I_, 0xF);
}
...
TYPED_TEST(ImplementationTest, Opcode_LD_B_Vx) {
  ...
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0xF0, 0x33);

  // Make sure the instruction succeeded.
  ASSERT_EQ(this->impl_.Step(), chip8::StepResult::kSuccess);

  // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index)
  ASSERT_EQ(this->impl_.memory_[this->impl_.I_ + 0], 1);

  // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index)
  ASSERT_EQ(this->impl_.memory_[this->impl_.I_ + 1], 2);

  // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index)
  ASSERT_EQ(this->impl_.memory_[this->impl_.I_ + 2], 3);
}

// NOLINTNEXTLINE(cppcoreguidelines-avoid-non-const-global-variables)
TYPED_TEST(ImplementationTest, Opcode_LD_B_Vx_DetectInvalidMemoryAddress) {
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  this->impl_.I_ = this->impl_.memory_.size() + 1;

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0xF0, 0x33);

  // Make sure the instruction did not succeed.
  ASSERT_EQ(this->impl_.Step(), chip8::StepResult::kInvalidMemoryLocation);
}
...
TYPED_TEST(ImplementationTest, Opcode_LD_I_Vx) {
  ...
  std::copy(this->impl_.memory_.cbegin(),
            this->impl_.memory_.cbegin() + this->impl_.V_.size(),
            this->impl_.V_.begin());

  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0xFF, 0x55);
  ...
  std::for_each(
      this->impl_.memory_.cbegin() + this->impl_.I_,
      this->impl_.memory_.cbegin() + this->impl_.I_ + this->impl_.V_.size(),
      [this, register_index = 0](const uint_fast8_t value) mutable {
        // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index)
        ASSERT_EQ(value, this->impl_.V_[register_index++]);
      });
}
...
TYPED_TEST(ImplementationTest, Opcode_LD_Vx_I) {
  ...
  // NOLINTNEXTLINE(cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers)
  InjectInstruction(this->impl_, 0xFF, 0x65);
  ...
  std::for_each(this->impl_.V_.cbegin(), this->impl_.V_.cend(),
                [this](const uint_fast8_t value) {
                  // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-constant-array-index)
                  ASSERT_EQ(value, this->impl_.memory_[this->impl_.I_++]);
                });
}